...
export const BookPage: React.FC<BookPageProps> = ({
  ...
  const buttonStyles = {
    light: isActive && (isPlaying || isLoading) ? 'bg-blue-100 text-blue-700' : 'bg-stone-100 text-stone-600 hover:bg-stone-200',
          ...
          {isActive && isLoading ? (
            ...
            <span className="flex items-center gap-2">
              <span className="relative flex h-3 w-3">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${theme === 'high-contrast' ? 'bg-yellow-400' : 'bg-blue-400'}`}></span>
                <span className={`relative inline-flex rounded-full h-3 w-3 ${theme === 'high-contrast' ? 'bg-yellow-500' : 'bg-blue-500'}`}></span>
              </span>
